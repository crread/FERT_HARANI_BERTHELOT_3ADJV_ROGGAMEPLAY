...
public class SystemUpdateMoveLinearComponent : IUpdater
{
    public void SystemUpdate()
    {
        ...
        foreach (LinearDeplacement module in accessor.GetAllModules())
        {
            if (module.gameObject.CompareTag("Player"))
            {
                ...
                if (playerPlane.Raycast(ray, out hitDist))
                {
                    ...
                    Quaternion targetRotation =
                        Quaternion.LookRotation(targetPoint - module.gameObject.transform.position);
                    targetRotation.x = 0;
                    targetRotation.z = 0;
                    module.gameObject.transform.rotation = Quaternion.Slerp(module.gameObject.transform.rotation,
                        targetRotation, 7f * Time.deltaTime);
                }
            ...
            if (module.gameObject.CompareTag("Ennemy"))
            {

                module.gameObject.transform.Translate((UnityEngine.Random.Range(5f,10f) * Time.deltaTime) * module.forward );
